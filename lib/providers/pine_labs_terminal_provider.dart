...
class PineLabsTerminalProvider with ChangeNotifier {
  ...
  Map<String, dynamic> _transactionHeader = PineLabsConfig.getTransactionHeader();
  ...
  Future<String> processSale({required double amount, required String billingRefNo}) async {
    // Validate Pine Labs credentials are configured
    if (!PineLabsConfig.areCredentialsConfigured()) {
      final errorMsg = 'Pine Labs credentials not configured. Please update ApplicationId and UserId in pine_labs_config.dart';
  ...
  Future<String> _startTransaction(Map<String, dynamic> payload) async {
    ...
    try {
      ...
      final result = await PineLabsTerminalService.startTransaction(jsonPayload);
