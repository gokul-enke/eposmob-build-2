...
class _AddProductStockScreenState extends State<AddProductStockScreen> {
  ...
  Future<void> _submitForm() async {
    ...
    try {
      ...
      debugPrint(
          "Retail Price: ${retailPriceController.text.isEmpty ? '0' : retailPriceController.text}");
      debugPrint(
          "Expiry Date: ${DateFormat('yyyy-MM-dd').format(selectedExpiryDate)}");
      ...
      final result = await Provider.of<StockProvider>(context, listen: false)
        ...
        retailPrice: retailPriceController.text.isEmpty
            ? '0.00'
            : double.parse(retailPriceController.text).toStringAsFixed(2),
        purchaseRate: purchaseRateController.text.isEmpty
            ? '0.00'
            : double.parse(purchaseRateController.text).toStringAsFixed(2),
        mrp: mrpController.text.isEmpty
            ? retailPriceController.text
            : mrpController.text,
        wholesalePrice: wholesalePriceController.text.isEmpty
            ? retailPriceController.text
        ...
        wholesaleMinUnit: wholesaleMinUnitController.text.isEmpty
            ? '1'
            : wholesaleMinUnitController.text,
        ...
        purchaseNumber: purchaseNumberController.text.isEmpty
            ? null
            : purchaseNumberController.text,
        taxInclude: taxInclude,
        initialRetailPrice: retailPriceController.text.isEmpty
            ? '0'
            : retailPriceController.text,
        initialWholesalePrice: wholesalePriceController.text.isEmpty
            ? retailPriceController.text
            : wholesalePriceController.text,
        retailPriceTax: null,
        wholesalePriceTax: null,
        context:
            context, // ✅ Pass context for manual LocalProductProvider updates
      );
      ...
      if (result['status'] == 'success') {
        showScaffold(
            context: context,
            message: result['message'] ?? 'Stock added successfully');
        ...
        if (mounted) {
          debugPrint(
              "🚀 Stock added successfully - LocalProductProvider updated manually (no API delay!)");
        }
  ...
  Widget _buildCategoryDropdown(Size size) {
    return Consumer<CategoryProvider>(
      builder: (context, categoryProvider, child) {
        List<category_models.Category>? categoryList =
  ...
  Widget _buildProductDropdown(Size size) {
    return Consumer<GridSelectionProvider>(
      builder: (context, gridProvider, child) {
        ...
        if (selectedProduct != null &&
            !uniqueProducts
                .any((p) => p.productId == selectedProduct!.productId)) {
                ...
                items: uniqueProducts.map((product) {
                  return DropdownMenuItem<GetProduct>(
                    ...
                    child: Text(
                      ...
                      overflow:
                          TextOverflow.ellipsis, // Handle long product names
                    ),
                  );
                }).toList(),
