...
class BillingPageRestaurant extends StatefulWidget {
  ...
  @override
  State<BillingPageRestaurant> createState() => BillingPageState();
}

class BillingPageState extends State<BillingPageRestaurant>
  ...
  void initState() {
    ...
    debugPrint(
        "🟡 [BillingPageRestaurant] Setting up barcode stream listener...");
    _barcodeSubscription = barcodeProvider.barcodeStream.listen((barcode) {
      debugPrint(
          "🟡 [BillingPageRestaurant] ========== BARCODE STREAM RECEIVED ==========");
      debugPrint("🟡 [BillingPageRestaurant] Barcode from stream: '$barcode'");
      debugPrint("🟡 [BillingPageRestaurant] Widget mounted: $mounted");
      if (mounted) {
        debugPrint("🟡 [BillingPageRestaurant] Calling processBarcode()...");
        processBarcode(barcode);
      } else {
        debugPrint(
            "⚠️ [BillingPageRestaurant] Widget not mounted - skipping processBarcode");
      }
      debugPrint(
          "🟡 [BillingPageRestaurant] =============================================\n");
    });
    debugPrint(
        "🟡 [BillingPageRestaurant] Barcode stream listener setup complete");
  ...
  Future<void> processBarcode(String barcode) async {
    debugPrint(
        "🔴 [BillingPageRestaurant.processBarcode] ========== PROCESS BARCODE START ==========");
    debugPrint(
        "🔴 [BillingPageRestaurant.processBarcode] Input barcode: '$barcode'");
    debugPrint(
        "🔴 [BillingPageRestaurant.processBarcode] Barcode length: ${barcode.length}");
    debugPrint(
        "🔴 [BillingPageRestaurant.processBarcode] _isProcessingBarcode: $_isProcessingBarcode");
    debugPrint(
        "🔴 [BillingPageRestaurant.processBarcode] barcode.isEmpty: ${barcode.isEmpty}");

    // If a barcode is already being processed, or if the input is empty, do nothing.
    if (_isProcessingBarcode || barcode.isEmpty) {
      debugPrint(
          "⚠️ [BillingPageRestaurant.processBarcode] SKIPPING - Already processing or empty barcode");
      debugPrint(
          "🔴 [BillingPageRestaurant.processBarcode] ========== PROCESS BARCODE END (SKIPPED) ==========\n");
      return;
    }

    debugPrint(
        "✅ [BillingPageRestaurant.processBarcode] Starting barcode processing...");
    // Set the flag to true to prevent duplicate processing.
    setState(() {
      _isProcessingBarcode = true;
    });
    debugPrint(
        "🔴 [BillingPageRestaurant.processBarcode] _isProcessingBarcode set to true");
    String query = barcode;

    List<GetProduct> filteredProducts = [];
    try {
      debugPrint(
          "🔴 [BillingPageRestaurant.processBarcode] Parsing barcode...");
      String? prefix;
      String? productCode;
      String? lastFive;
      ...
      if (query.length > 2) {
        ...
        debugPrint("🔴 [BillingPageRestaurant.processBarcode] Prefix: $prefix");
      }

      if (prefix != '000' || query.length != 14) {
        debugPrint(
            "🔴 [BillingPageRestaurant.processBarcode] Standard barcode - searching by: '$query'");
        filteredProducts =
            Provider.of<LocalProductProvider>(context, listen: false)
        ...
        debugPrint(
            "🔴 [BillingPageRestaurant.processBarcode] Weight/Count barcode - productCode: $productCode, lastFive: $lastFive");
        filteredProducts =
            Provider.of<LocalProductProvider>(context, listen: false)
                .filterProductByBarcode(
          barCode: productCode,
      ...
      if (filteredProducts.isNotEmpty) {
        ...
        debugPrint(
            "🔴 [BillingPageRestaurant.processBarcode] 🛒 Calling ProductCartHelper with:");
        debugPrint(
            "🔴 [BillingPageRestaurant.processBarcode]   - Product: ${product.productName}");
        debugPrint(
            "🔴 [BillingPageRestaurant.processBarcode]   - Quantity: $quantity");
        debugPrint(
            "🔴 [BillingPageRestaurant.processBarcode]   - Customer ID: $selectedCustomerID");
        debugPrint(
            "🔴 [BillingPageRestaurant.processBarcode]   - Customer Name: ${selectedCustomer?.name}");
        ...
        debugPrint(
            "✅ [BillingPageRestaurant.processBarcode] Product added to cart successfully");

        // Clear input fields
        debugPrint(
            "🔴 [BillingPageRestaurant.processBarcode] Clearing input fields...");
        setState(() {
        ...
        debugPrint(
            "🔴 [BillingPageRestaurant.processBarcode] Input fields cleared");
        _focusTextField();
        debugPrint("🔴 [BillingPageRestaurant.processBarcode] Focus reset");
      } else {
        debugPrint(
            "⚠️ [BillingPageRestaurant.processBarcode] No products found for barcode: '$query'");
        ...
        debugPrint(
            "🔴 [BillingPageRestaurant.processBarcode] No products found for barcode: '$query'");
      }
    } catch (e) {
      debugPrint("❌ [BillingPageRestaurant.processBarcode] ERROR: $e");
      debugPrint(
          "❌ [BillingPageRestaurant.processBarcode] Stack trace: ${StackTrace.current}");
      showScaffoldError(
        context: context,
      ...
      debugPrint(
          "🔴 [BillingPageRestaurant.processBarcode] Barcode cleared after error");
    } finally {
      debugPrint(
          "🔴 [BillingPageRestaurant.processBarcode] Finally block - resetting processing flag...");
      // Reset the flag and ensure barcode is always cleared
      Future.delayed(const Duration(milliseconds: 750), () {
        if (mounted) {
          debugPrint(
              "🔴 [BillingPageRestaurant.processBarcode] Resetting _isProcessingBarcode to false");
          setState(() {
          ...
          debugPrint(
              "🔴 [BillingPageRestaurant.processBarcode] Processing complete - ready for next scan");
        }
      });
      debugPrint(
          "🔴 [BillingPageRestaurant.processBarcode] ========== PROCESS BARCODE END ==========\n");
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    debugPrint("🔨 BillingPageRestaurant build() called");
    debugPrint(
                          ...
                          child: Padding(
                            ...
                            child: Column(
                              children: [
                                ...
                                // Quick Access Products section (compact horizontal scroll)
                                SizedBox(
                                  height: 90,
                                  child: Consumer<GridSelectionProvider>(
                                    builder: (context, gridProvider, child) {
                                      final products =
                                          gridProvider.quickAccessProductList ??
                                              [];
                                      if (products.isEmpty) {
                                        ...
                                        itemBuilder: (context, index) {
                                          ...
                                          return GestureDetector(
                                            ...
                                            child: Container(
                                              ...
                                              child: Column(
                                                children: [
                                                  // Product image with info button (compact)
                                                  Expanded(
                                                    flex: 3,
                                                    child: Stack(
                                                      children: [
                                ...
                                // Commented out _buildOrderHeader - not needed in restaurant screen
                                // _buildOrderHeader(
                                //   size: size,
  ...
  // ============ NEW RESTAURANT LAYOUT METHODS ============

  /// Helper widget for non-editable cart item field
  Widget _buildCartItemField({
    required String label,
  ...
  /// Helper widget for editable cart item field
  Widget _buildCartItemFieldEditable({
    required String label,
    required String value,
    required VoidCallback onTap,
    CrossAxisAlignment crossAxisAlignment = CrossAxisAlignment.start,
        ...
        children: [
          ...
          Row(
            ...
            children: [
              ...
              const SizedBox(width: 4),
              Icon(Icons.edit, size: 9, color: Colors.grey.shade500),
            ],
          ),
        ],
      ),
  ...
  void _showEditCartItemDialog({
    ...
    switch (field) {
      case 'price':
        title = 'billing.edit_price'.tr;
        currentValue = (item.price ?? 0.0).toStringAsFixed(2);
        labelText = 'billing.table_price'.tr;
        prefix = '$currency ';
        break;
      case 'mrp':
        title = 'billing.table_mrp'.tr;
        currentValue = (item.mrp ?? 0.0).toStringAsFixed(2);
        labelText = 'billing.table_mrp'.tr;
        prefix = '$currency ';
        break;
      case 'tax':
        title = 'billing.table_tax'.tr;
        currentValue = (item.taxRate ?? 0.0).toStringAsFixed(1);
        labelText = 'billing.table_tax'.tr;
        suffix = '%';
        break;
      default:
        return;
  ...
  void _showEditPriceDialog({
    ...
    final currentValue = (item.price ?? 0.0).toStringAsFixed(2);
          ...
          children: [
            ...
            TextField(
              ...
              keyboardType:
                  const TextInputType.numberWithOptions(decimal: true),
              decoration: InputDecoration(
                labelText: 'billing.unit_price'.tr,
                prefixText: '$currency ',
                border: const OutlineInputBorder(),
              ),
              autofocus: true,
          ...
          ElevatedButton(
            onPressed: () {
              ...
              if (newPrice != null && newPrice >= 0) {
                // Update the cart item price using provider method
                localProductProvider.updateProductPricingInCart(
                  item.product.productId!,
                  newPrice,
                  newPrice, // mrp same as price
                  item.taxRate ?? 0.0,
                );
                Navigator.pop(dialogContext);
              }
  ...
  // State variables for main product grid
  final TextEditingController _mainProductSearchController =
      TextEditingController();
  int _selectedMainCategoryIndex = 0;

  /// Compact cart sidebar for restaurant layout
  Widget _buildCompactCartSidebar() {
    return Consumer<LocalProductProvider>(
      builder: (context, localProductProvider, child) {
  ...
  /// Main product grid for restaurant layout - shows categories and products in main area
