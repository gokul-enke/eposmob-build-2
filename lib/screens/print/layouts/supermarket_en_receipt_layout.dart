...
class SupermarketEnReceiptLayout implements ReceiptLayout {
  ...
  @override
  String get layoutId => 'supermarket_en';

  @override
  String get displayName => 'Supermarket En';

  @override
  Future<void> printThermal(ReceiptLayoutParams params) async {
    debugPrint("===== SUPERMARKET EN LAYOUT: THERMAL PRINTING ====");
  ...
  void _buildFinalSummarySection(
    ...
    final purchaseLabel = _getLabel(displayConfig, 'showFinalPurchase', null,
        isEnglish ? 'ORDER TOTAL' : 'إجمالي الطلب');
    final returnLabel = _getLabel(displayConfig, 'showFinalReturn', null,
        isEnglish ? 'RETURN TOTAL' : 'إجمالي المرتجع');
    final finalLabel = _getLabel(displayConfig, 'showFinalNetAmount', null,
...
// Label on Left
        ...
        // Icon/symbol + Value on Right
        double valueRightAnchor = width - padding;

        if (item.icon != null) {
          final double iconSize = itemFontSize * 0.75;
          // Draw value first to know where icon goes
          ...
          final double valueX = valueRightAnchor - valuePainter.width;
          ...
          final double symbolX = valueRightAnchor - symbolPainter.width;
          ...
          _drawScaledText(
            canvas,
            item.value,
            Offset(valueRightAnchor, currentY),
            width * 0.40,
            itemFontSize,
            item.isBold,
            TextAlign.right,
