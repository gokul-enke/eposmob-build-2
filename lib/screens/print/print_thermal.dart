...
class ThermalPrinter {
  ...
  List<int> _buildHeader(
    ...
    // Detect paper size for dynamic font sizing
    // Note: We don't have paper size here, so we use larger sizes by default
    // In a future refactor, consider passing paper size to this method
    const bool is58mm = false; // Default to 80mm sizing for header
        ...
        bytes += generator.row([
          PosColumn(
            ...
            styles: PosStyles(
                ...
                align: PosAlign.center,
                bold: true,
                width: getHeaderSize(is58mm),
                height: getHeaderSize(is58mm)),
          ),
        ]);
      }
    }
    ...
    // Invoice Title (from DocumentConfig header or displayConfig value) - MEDIUM IMPORTANCE
    if (displayConfig?['showInvoiceTitle']?.visible == true) {
      final invoiceTitle =
          displayConfig?['showInvoiceTitle']?.value as String? ??
              docConfig?.header ??
      ...
      if (invoiceTitle.isNotEmpty) {
        bytes += generator.text(invoiceTitle,
            styles: PosStyles(
              ...
              align: PosAlign.center,
              bold: true,
              height: getSubtitleSize(is58mm),
              width: getSubtitleSize(is58mm),
            ));
      }
    }

    // Invoice Number - MEDIUM IMPORTANCE (uses subtitle size for readability)
    if (displayConfig?['showInvoiceNumber']?.visible == true) {
  ...
  List<int> _buildTotalAmount(
    ...
    if (displayConfig?['showNetAmount']?.visible == true) {
      rightSideItems.add({
        ...
        'textSize': 'big', // HIGH IMPORTANCE - use larger size
        'bold': 'true',
