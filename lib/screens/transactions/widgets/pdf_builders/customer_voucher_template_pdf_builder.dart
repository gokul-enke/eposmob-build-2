...
/// Builds an A4 PDF that matches the Customer Voucher template design
/// from the Document Templates web dashboard.
///
/// Uses [DocumentConfig] (from `document-configs?type=voucher`) for styling.
/// Uses [CustomerVoucher] for actual voucher data.
class CustomerVoucherTemplatePdfBuilder {
  /// Generates the Customer Voucher template PDF and returns the [File].
  ///
  /// Returns `null` if PDF generation fails.
  static Future<File?> generate({
    required CustomerVoucher details,
    ...
    try {
      ...
      // ── Header info ───────────────────────────────────────────
      final String documentTitle = config.header ?? 'Voucher';
  ...
  /// "From:" (company) on the left, "Pay To:" (customer) on the right
  ...
  static pw.Widget _buildSignatures(CustomerVoucher details) {
    return pw.Container(
      alignment: pw.Alignment.centerRight,
      child: pw.Column(
