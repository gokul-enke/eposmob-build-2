...
class CustomCalendarPickerTableCell extends StatefulWidget {
  ...
  _CustomCalendarPickerTableCellState createState() =>
      _CustomCalendarPickerTableCellState();
}
...
class _CustomCalendarPickerTableCellState
    extends State<CustomCalendarPickerTableCell> {
  ...
  Future<void> _selectDate(BuildContext context) async {
    final DateTime initialDate =
        widget.initialDate ?? selectedDate ?? DateTime.now();
      ...
      if (parsedDate != null) {
        ...
        if (parsedDate.isBefore(firstDate) || parsedDate.isAfter(lastDate)) {
          _showDateValidationError(
              'Date must be between ${DateFormat('MMM dd, yyyy').format(firstDate)} and ${DateFormat('MMM dd, yyyy').format(lastDate)}');
          return;
        }
        ...
        _showDateValidationError(
            'Invalid date. Try: 20250205 or 2025-02-05 or 05-02-2025');
      }
    } catch (e) {
      _showDateValidationError(
          'Invalid date. Try: 20250205 or 2025-02-05 or 05-02-2025');
          ...
          content: Container(
            ...
            child: Column(
              ...
              children: [
                ...
                _buildQuickDateButton(
                    '1 Month', DateTime.now().add(const Duration(days: 30))),
                _buildQuickDateButton(
                    '3 Months', DateTime.now().add(const Duration(days: 90))),
                _buildQuickDateButton(
                    '6 Months', DateTime.now().add(const Duration(days: 180))),
                _buildQuickDateButton(
                    '1 Year', DateTime.now().add(const Duration(days: 365))),
            ...
            Expanded(
              child: isTextInputMode && widget.allowTextInput
                  ? TextFormField(
                      ...
                      inputFormatters: [CustomDateInputFormatter()],
...
class CustomTimePickerTableCell extends StatefulWidget {
  final Function(TimeOfDay) onTimeSelected;
  final TimeOfDay? initialTime;

  const CustomTimePickerTableCell({
    Key? key,
  ...
  @override
  _CustomTimePickerTableCellState createState() =>
      _CustomTimePickerTableCellState();
}

class _CustomTimePickerTableCellState extends State<CustomTimePickerTableCell> {
